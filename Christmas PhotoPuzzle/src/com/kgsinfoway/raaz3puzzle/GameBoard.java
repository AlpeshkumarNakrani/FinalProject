 ...
 * user has correctly solved the puzzle. TileViews are stationary. Tiles move
 * around and are given to TileViews to display.
 * 
 * @author wadechatam
 * 
 */
public final class GameBoard {
	...
	private GameBoard(Context context, Bitmap bitmap, TableLayout parentLayout,
			int width, int height, short gridSize, TextView moveTextView) {
	...
	/*
	 * (non-Javadoc) Cut the picture into pieces and assign it to tiles.
	 */
	...
	/*
	 * (non-Javadoc) Determines if the current tile arrangement is solvable.
	 * This is mathematically provable by determining the number of incorrectly
	 * ordered tiles (permutations). If the number of permutations plus taxicab
	 * distance of the blank tile is even, then the puzzle is solvable.
	 * Pseudo-code for algorithm: Start with the first tile on the board
	 * (top-left) Determine its correct location For all of the tiles that
	 * follow it on the board, determine if they should come before the current
	 * tile. If so, that tile is considered out of order and should increment
	 * the out-of-order count. Repeat for following tiles. If out-of-order count
	 * + row number of blank is even, it is solvable To better understand the
	 * code, it is easier to quickly read the comments below completely, then go
	 * back and read the code. For game consistency, the last tile
	 * (bottom-right) is always the blank. Putting the blank tile at its correct
	 * location reduces its taxicab distance to 0, so its row number can be
	 * ignored.
	 * 
